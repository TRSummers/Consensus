...
class VerizonNewActivationSimulation extends Simulation {
  ...
  setUp(VerizonNewActivationE2EComponents.VerizonNewActivationE2EFlow.inject(atOnceUsers(1)).protocols(SimParams.httpProtocol))
//    setUp(VerizonCarrierResponderComponents.VerizonCR.inject(atOnceUsers(1)).protocols(SimParams.httpProtocol))

}

object VerizonNewActivationE2EComponents {

  val VerizonNewActivationE2EFlow = scenario("Verizon New").repeat(SimParams.inum) {
